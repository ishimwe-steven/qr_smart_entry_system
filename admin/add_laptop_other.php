...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stmt_check->num_rows > 0) {
        ...
        $stmt = $conn->prepare("INSERT INTO laptops (owner_type, other_id, brand, serial_number) VALUES ('other', ?, ?, ?)");
        $stmt->bind_param("iss", $other_id, $brand, $serial_number);
...
// Fetch all "others" (lecturers, staff, guests)
$others = $conn->query("SELECT id, first_name, last_name, role, department FROM others ORDER BY role, first_name, last_name");
?>
...
<style>
    ...
    h2 { font-size: 2.5rem; font-weight: 700; text-shadow: 0 2px 4px rgba(0, 0, 0, 0.2); margin-bottom: 2rem; }
    ...
    @media (max-width: 768px) {
        .flex-grow-1 { margin-left: 0; padding: 1rem; }
        form { max-width: 100%; margin: 60px auto 0 auto; padding: 1rem; }
        h2 { font-size: 2rem; }
    }
</style>
...
<div class="d-flex">
  ...
  <div class="flex-grow-1 p-4">
    <h2>Add Laptop for Others</h2>

    <?= $message ?>

    <form method="POST" style="max-width:600px;">
      <div class="mb-3">
        <label class="form-label">Select Person (Lecturer / Staff / Guest)</label>
        <select name="other_id" class="form-select" required>
          <option value="">-- Select Person --</option>
          <?php while ($row = $others->fetch_assoc()): ?>
            <option value="<?= $row['id'] ?>">
              <?= htmlspecialchars($row['first_name'] . ' ' . $row['last_name']) ?> 
              (<?= htmlspecialchars($row['role']) ?>, <?= htmlspecialchars($row['department']) ?>)
            </option>
          <?php endwhile; ?>
        </select>
      </div>

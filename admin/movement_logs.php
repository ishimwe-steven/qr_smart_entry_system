...
// Fetch all movement logs (students + others)
$sql = "
  SELECT 
      m.id, 
      m.status, 
...
<style>
  ...
  h2 { font-size: 2.5rem; font-weight: 700; text-shadow: 0 2px 4px rgba(0,0,0,0.2); color: black; }
  .table-responsive {
    width: 97%;
  ...
  .dataTables_wrapper .dataTables_filter label { color: #2c3e50; font-weight: 500; }
        ...
        <tbody>
          <?php while ($row = $result->fetch_assoc()): ?>
            <tr>
              ...
              <td>
                <a href="delete_movement.php?id=<?= $row['id'] ?>" 
